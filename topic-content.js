/*
  PhysicsPath — topic-content.js
  ---------------------------------
  Single source of truth for every topic page's content.
  topic.html reads an entry from here based on the URL (?t=topic-id)
  and builds the whole page — so adding a new topic to the site means
  adding ONE object below, nothing else.

  Structure per topic:
    moduleTitle: which module it belongs to (shown in the breadcrumb)
    title: topic name (shown as page title and heading)
    levels: array of 4 HTML strings — [Level 0, Level 1, Level 2, Level Max]

  Currently filled: rest of Module 1 (Oscillations & Waves).
  Simple Harmonic Motion has its own standalone page (simple-harmonic-motion.html)
